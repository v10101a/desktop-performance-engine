...
/// Snapshot of the pre-show desktop state: the cursor location. (Icon positions and
/// the wallpaper are snapshotted by their own controllers.)
...
final class RestoreManager {
    ...
    /// Set true once an effect actually moves the real cursor. Until then we never
    /// warp it on restore, so panic can't nudge a cursor we never touched.
