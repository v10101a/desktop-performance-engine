...
| 27 | CHORUS 2A − 1.0 | f 3990 · beat 284 | The vocal comes in (bar 72, the pickup bar before chorus 2A): a **ton of crazy UI windows**, spammed; its lyric cards are ALL CAPS like the spiral's. One of the small windows **is running DooM** — 320×200, lower left, the 1997 linuxdoom sources built to wasm32, playing their own attract-mode demo until the stop closes it at f 4817. It opens here rather than at cue 28 because DooM holds its title screen for ~5 s, and it keeps its own window id: the eruption recycles `w0…w13` every few beats, and a re-opened id restarts the game. The engine is fetched, not committed — `tools/fetch_doom.sh`; without it the window says so. | the eruption: `openWindow`/`fakeDialog`/`jiggle` + `doom` |
