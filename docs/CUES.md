...
| 21 | INSTRUMENTAL A + 3.1 | f 3331 · beat 237 | Everything cuts to the bare desktop and the desktop goes **blue** (it stays that way to the end card). **The horse is cut** (2026-09-03) and the **segmenter swarm** is in its place: `assets/giveit2meclip.mov` segmented for **motion**, every region that moves becoming its own titled window holding the piece of frame it was cut from, pinned where it was found. Nothing is tracked between frames, so a thing that keeps moving mints a new window every frame and the screen fills — up to 60, then the oldest panel is recycled. It reads as the sentence the horse read as, the desktop taken over by one moving thing, in the show's own furniture rather than a sprite. The horse act is kept behind `HORSE_ACT = False`. | `deskWallpaper` solid + `segSwarm` |
