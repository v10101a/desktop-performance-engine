...
| 15 | BRIDGE B + 1.2 | f 2337 · beat 166 | Windows start opening, **slowly filling the screen**. Five of the 13 flat cards come up broken instead, interleaved: three **torn** (pixelface f 2488, the horse f 2570, the credits tile f 2604 — the same glitch pass the desktop gets at cue 28, once each, tearing harder as it goes: 0.45 → 0.63 → 0.81) and two **Wolfram automata actually running** — black on white, scrolling a generation at a time (rule 30 f 2506, rule 110 f 2588). No lyric popups: the alert-dialog slots are `haunt.sh` terminals now, so the fill keeps its density without quoting the lyric. The fill decays as it thickens. | ramped `openWindow` + `glitch` / `automaton` content |
| 16 | BRIDGE B + 7.0 | f 2645 · beat 188 | The bar before the breakdown: the desktop **fades to black**; the windows close **one by one**, and the stagger crosses the section line — the last few closes land ~1.7 beats after cue 17, so the raymarcher opens as the stragglers are still leaving instead of after a beat of dead air. | `deskWallpaper` solid `#000000` + staggered `closeWindow` |
